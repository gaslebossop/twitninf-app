...
function ChatRow({ item }: { item: ChatMessage }) {
  ...
  const hue = (Array.from(item.user).reduce((acc, c) => acc + c.charCodeAt(0), 0) * 37) % 360;
  const nameColor = `hsl(${hue}, 70%, 82%)`;
...
const styles = StyleSheet.create({
  ...
  chatBubble: { backgroundColor: 'rgba(0,0,0,0.5)', borderRadius: 16, paddingHorizontal: 12, paddingVertical: 8, flexShrink: 1 },
  chatHeader: { flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 2 },
  chatUsername: { fontSize: 12, fontWeight: '700' },
  chatText: { color: '#fff', fontSize: 14, lineHeight: 18 },
