/**
 * Système de style global qui s'applique automatiquement à tous les composants
 */
...
let styleOverrideCache: Map<string, any> = new Map();
let currentThemeId: string | null = null;

// Fonction pour créer des styles override
const createStyleOverrides = (theme: any) => {
  return {
...
export const getStyleOverrides = (themeId?: string) => {
  if (!themeId) return {};

  // Si le thème a changé, recréer les styles
  if (currentThemeId !== themeId || !styleOverrideCache.has(themeId)) {
    const theme = getEventTheme(themeId);
    if (theme) {
      const styleOverrides = createStyleOverrides(theme);
      styleOverrideCache.set(themeId, styleOverrides);
      currentThemeId = themeId;
    }
  }

  return styleOverrideCache.get(themeId) || {};
};
...
export const overrideStyle = (originalStyle: any, themeId?: string, componentType?: string) => {
  ...
  const styleOverrides = getStyleOverrides(themeId);
  if (!styleOverrides) return originalStyle;

  // Obtenir le style override pour ce type de composant
  let overrideStyle = {};
  if (componentType && styleOverrides[componentType]) {
    overrideStyle = styleOverrides[componentType];
  }
